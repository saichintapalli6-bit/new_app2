...
def sellerdetails(request):
    if not request.session.get('admin'):
        return render(request,'adminLoginForm.html') 
    users=sellerRegisteredTable.objects.all()
    return render(request,'admin/sellerdetails.html',{'users':users})




def Sactivate_user(request):
    if not request.session.get('admin'):
        return render(request,'adminLoginForm.html') 

    id=request.GET['id']
    user = get_object_or_404(sellerRegisteredTable, id=id)
    user.status = 'Active'
    user.save()
    users=sellerRegisteredTable.objects.all()
    return render(request,'admin/sellerdetails.html',{'users':users})  

def Sdeactivate_user(request):
    if not request.session.get('admin'):
        return render(request,'adminLoginForm.html') 
